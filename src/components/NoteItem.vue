<template>
  <div class="note"
    ...
    <div class="note__controls">
      ...
      <image-input @change="value => changeNote(value, 'image')" title="Add image"/>

      <app-icon v-if="isList" type="text" @click="transformToText" title="Transform to text"/>
      <app-icon v-if="!isList" type="list" @click="transformToList" title="Transform to list"/>

      <app-icon type="delete" @click="deleteNote(form.id)" title="Delete note"/>

      <app-icon v-if="fullViewMode" :disabled="!withContent" type="save" @click="closeForm" class="save-button"/>
      <app-icon v-if="!fullViewMode" type="edit" @click="openForm(form)" title="Edit note"/>
    </div>
  </div>
</template>
...
<script>
    ...
    export default {
        ...
        beforeDestroy() {
            ...
            if (!note.id && this.withContent) this.createNote(note);
        },
        methods: {
            ...mapMutations([
                'updateNoteProperty',
